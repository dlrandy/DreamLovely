...
##什么时候需要使用原型模式？
1. 在不能找到要copy对象的实体类的时候
2. 在想要减少只在初始化时不同的子类数量的时候
  就是预置一些对象作为原型，用户只需要找到适当的原型并克隆它，而不是实例化
  一个匹配配置的子类。
...
###和其他模式的关系
1.当需要保存Command的拷贝到历史记录里的时候特别有用
  当重度使用了Composite和Decorator的时候，Prototype使得复杂对象的创建更容易
  而不是重新构成每个对象
2.Prototype不是基于继承的，但是它需要克隆对象的复杂初始化。工厂方法基于继承，
  但是不需要初始化步骤
